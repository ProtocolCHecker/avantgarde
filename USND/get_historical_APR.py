...
#         # Market configurations from your original script
#         self.MARKETS = {
#             'WETH': {
#                 'display_name': 'WETH',
#                 'token_tag': 'ETH',
...
#         # Special handling for WETH tables (based on your original script)
...
# def calculate_strategy_indices(market_data: Dict[str, pd.DataFrame], analytics: NeriteSuperAPRAnalytics, tbtc_collateral: float, ltv_percent: float) -> pd.DataFrame:
#     """Calculate strategy-based APR indices with historical price data and adjusted APRs"""
...
#         tbtc_prices_df = analytics.get_historical_prices("tbtc")
#         usnd_prices_df = analytics.get_historical_prices("us-nerite-dollar")
    
#     if tbtc_prices_df.empty:
#         st.error("Failed to fetch tBTC price data")
#         return pd.DataFrame()
...
#                 # APR = (Interest to SP * 0.75) / New Pool Size * 100
#                 # Since interest remains constant, APR decreases as pool size increases
...
#                     adjusted_apr_tvl = 0
#                     tvl_allocation = 0
#                     new_pool_size_tvl = item['original_pool_size']
...
#                 # Store additional data for debugging/analysis
#                 item['equal_allocation'] = equal_allocation_per_market
#                 item['tvl_allocation'] = tvl_allocation if 'tvl_allocation' in locals() else 0
#                 item['new_pool_size_equal'] = new_pool_size_equal
#                 item['new_pool_size_tvl'] = new_pool_size_tvl
#                 item['adjusted_apr_equal'] = adjusted_apr_equal
#                 item['adjusted_apr_tvl'] = adjusted_apr_tvl
...
#                 'equal_allocation_apr': strategy_1_apr,  # Adjusted APR after our deposits
#                 'tvl_weighted_allocation_apr': strategy_2_apr,  # Adjusted APR after our deposits
#                 'original_equal_apr': original_equal_apr,  # Original APR before our deposits
#                 'original_tvl_apr': original_tvl_apr,  # Original APR before our deposits
...
#                 'apr_impact_equal': original_equal_apr - strategy_1_apr,  # How much APR decreased
#                 'apr_impact_tvl': original_tvl_apr - strategy_2_apr  # How much APR decreased
#             })
    
#     return pd.DataFrame(strategy_data)

# def create_apr_dashboard(market_data: Dict[str, pd.DataFrame], analytics: NeriteSuperAPRAnalytics):
#     """Create the main APR dashboard with charts"""
    
#     st.header("📈 Nerite Protocol Stability Pool APR Dashboard")
#     st.markdown("Historical APR analysis across all Nerite Protocol markets")
...
#         # Calculate indices
#         indices_df = calculate_apr_indices(active_markets)
...
#             # Index data table
#             st.subheader("📋 Historical Index Data")
            
#             display_indices_df = indices_df[['date', 'equal_weighted_apr', 'tvl_weighted_apr', 'total_tvl', 'markets_count']].copy()
#             display_indices_df.columns = ['Date', 'Equal-Weighted APR (%)', 'TVL-Weighted APR (%)', 'Total TVL (USND)', 'Markets Count']
#             display_indices_df['Date'] = display_indices_df['Date'].dt.strftime('%Y-%m-%d')
#             display_indices_df['Total TVL (USND)'] = display_indices_df['Total TVL (USND)'].apply(lambda x: f"{x:,.0f}")
            
#             st.dataframe(display_indices_df.head(30), use_container_width=True)
...
#         strategy_df = calculate_strategy_indices(active_markets, analytics, tbtc_collateral, ltv_percent)
...
#                 yield_diff_usnd = annual_yield_equal_usnd - annual_yield_tvl_usnd
...
#                 # tBTC price chart
#                 fig_tbtc = go.Figure()
#                 fig_tbtc.add_trace(go.Scatter(
#                     x=strategy_df['date'],
#                     y=strategy_df['tbtc_price'],
#                     mode='lines',
#                     name='tBTC Price',
#                     line=dict(color='#F7931A', width=2)
#                 ))
#                 fig_tbtc.update_layout(
#                     title="tBTC Price History",
#                     xaxis_title="Date",
#                     yaxis_title="Price (USD)",
#                     height=400,
#                     template="plotly_white"
#                 )
#                 st.plotly_chart(fig_tbtc, use_container_width=True)
            
#             with col2:
#                 # USND price chart
#                 fig_usnd = go.Figure()
#                 fig_usnd.add_trace(go.Scatter(
#                     x=strategy_df['date'],
#                     y=strategy_df['usnd_price'],
#                     mode='lines',
#                     name='USND Price',
#                     line=dict(color='#4ECDC4', width=2)
#                 ))
#                 fig_usnd.update_layout(
#                     title="USND Price History",
#                     xaxis_title="Date",
#                     yaxis_title="Price (USD)",
#                     height=400,
#                     template="plotly_white"
#                 )
#                 st.plotly_chart(fig_usnd, use_container_width=True)
...
#                     'Interest Cost': current_record['total_annual_interest_cost']
#                 })
...
#         progress_bar.progress((i) / len(all_markets))
...
#     st.success(f"🎯 Successfully loaded data for {loaded_markets}/{len(all_markets)} markets")
...
#     # Create dashboard
#     create_apr_dashboard(market_data, analytics)
...
#         st.markdown(f"**Markets Loaded:** {markets_loaded}/{len(all_markets)}")
